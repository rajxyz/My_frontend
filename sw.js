...
// ✅ App Badging - Show Notification Dot When Updates Are Available
async function updateBadge() {
  if ('setAppBadge' in navigator) {
    let unreadCount = await fetch('/get-unread-count').then(res => res.json());
    navigator.setAppBadge(unreadCount);
  }
}

// ✅ Remove App Badge When Notification Clicked
...
// ✅ Background Fetch - Download Large Translation Files in the Background
self.addEventListener('backgroundfetchsuccess', (event) => {
  console.log('Background fetch completed:', event.registration.id);
});
